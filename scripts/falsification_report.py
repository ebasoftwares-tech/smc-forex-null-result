...
def main() -> int:
    ...
    w("")
    w("### 4. Two guards nothing in the fixture reaches, and one that real bars reach hard")
    w("")
    ...
    w(f"| `choch.max_reference_distance_atr` in `choch_only` | On the **fixture** it rejects "
      "nothing -- the widest reference there sits at about 2.8 ATR. **On real bars it is "
      f"the opposite**: at the same FROZEN default of {cfg.choch.max_reference_distance_atr} "
      "it rejects **34.6%** of the references it sees |")
    w("")
    w("The third is worth separating from the other two, and it is the one this table got")
    w("wrong. It is a **measurement, not an arithmetic impossibility** like D-014's four")
    w("unreachable defaults -- and a measurement taken on a fixture does not transfer.")
    w("D-028 section 6 flagged the old claim as unverified rather than repeat it with")
    w("confidence; measuring it settled the matter in the other direction (D-032,")
    w("`scripts/verify_reference_guard.py`, which anyone can re-run in about eight")
    w("minutes):")
    w("")
    w("| | fixture | real bars, 10 symbols 2019-2022 |")
    w("|---|---:|---:|")
    w("| references evaluated | -- | 112,264 |")
    w("| widest | ~2.8 ATR | **15.01 ATR** |")
    w("| 99th percentile | -- | 7.58 ATR |")
    w(f"| rejected at {cfg.choch.max_reference_distance_atr} | **none** | **38,871 (34.6%)** |")
    w("")
    w("**The arm's numbers are unaffected**, because the guard was applied exactly as")
    w("specified while they were computed -- what was wrong was this report's description")
    w("of what it does, not the run. But `choch_only` is a far more filtered arm than the")
    w("old sentence implied, and any reading of it should account for a third of its")
    w("candidate references never reaching the break test at all.")
    w("")
    w("It is an ABLATION parameter over {2.0, 3.0, 4.0}, and **at 2.0 it binds hard** on")
    w("the fixture too -- which is how the branch is tested. It also echoes STATE.md")
    w("section 3: the Phase 9 gate is not robust to this same parameter. The fixture had it")
    w("sitting just past where the data reached; real data reaches five times past it.")
    w("")
