...
the assumption would be expensive.
    ...
    python scripts/marginal_value_report.py
"""
...
# Phase 9's projection, for the power arithmetic to be read against.
IS_YEARS, UNIVERSE, DEV_SET = 4, 10, 3
#: Phase 9's own per-symbol-year MSS rate, after SPEC 9.4 cluster dedup.
PHASE9_MSS_PER_YEAR = 38 / 3
...
def main() -> int:
    ...
    for k, year in enumerate(YEARS):
        ...
        h4, cands = build_year(cfg, year, seed=41 + k)
        per_year.append((year, h4, cands, MV.run_study(h4, cands, cfg)))
    ...
    per_symbol_year = pooled.n_mss / len(YEARS)
    proj_universe = per_symbol_year * UNIVERSE * IS_YEARS
    proj_dev = per_symbol_year * DEV_SET * IS_YEARS
    ...
    w(f"- Fixture: {len(YEARS)} synthetic years ({YEARS[0]}-{YEARS[-1]}), EURUSD, H4, `reference_mode = major`")
    ...
    if pooled.verdict() is MV.Verdict.UNDERPOWERED:
        ...
        w(f"zero. On this fixture it does not, at {n_under} of the {len(primary)} horizons.")
    ...
    w("them contribute the identical number more than once. On this fixture that halves")
    ...
    w(f"{PHASE9_MSS_PER_YEAR:.1f} MSS per symbol-year after *cluster* dedup (SPEC 9.4);")
    ...
    w("")
    w(f"| h | MSS needed for +/-{MV.EQUIVALENCE_MARGIN_ATR:g} ATR | Phase 9 projects (dev, {DEV_SET}x{IS_YEARS}y) | Enough? | Phase 9 projects (universe, {UNIVERSE}x{IS_YEARS}y) | Enough? |")
    w("|---:|---:|---:|:--:|---:|:--:|")
    for c in primary:
    ...
    if proj_universe < longest.required_mss_n:
        w(f"**At h={longest.horizon} the full in-sample universe is not enough.** Phase 9")
        w(f"projects {proj_universe:,.0f} MSS events across {UNIVERSE} symbols over")
    ...
    w("## Per-year stability")
    w("")
    w("| Year | CHoCH | MSS | h=1 diff | h=4 diff | h=12 diff |")
    w("|---|---:|---:|---:|---:|---:|")
    ...
    w("")
    w(f"## Status: {'PASS' if all_ok else 'FAIL'} — instrument validated, H5 open pending real data")
