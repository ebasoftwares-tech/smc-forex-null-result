...
def main() -> int:
    ...
    w("`OPPOSING_SWEEP` duly reads +0.002 ATR at a 45% hit rate. **`ENTRY_EXPIRED` does not,")
    ...
    w("2. **That the funnel's conversion rates transfer.** Armed-to-filled in particular is")
    w("   dominated by the opposing-sweep cancel at a sweep rate no real market sustains.")
    ...
    w("   constructed tests and are the first things to re-measure on real bars.")
    w("4. **The falsification suite (protocol 6).** Shuffled liquidity, sweep-only,")
    w("   CHoCH-only, reversed-order and random-time controls are the most informative runs")
    w("   in the project and are **not built**. They are studies, not engine, and every one")
    w("   of them answers a question that is meaningless on a fixture whose true effect is")
    w("   zero by construction. They are Phase 14's largest deliberate omission.")
    w("5. **Walk-forward (protocol 8) and the OOS budget ledger (protocol 7).** Both are")
    w("   procedures over real splits. There is no out-of-sample period to spend budget on.")
    w("6. **The pre-registration (protocol 1).** Due before the first *strategy* backtest,")
    w("   which this is not — a synthetic run validates an instrument. Writing it is the")
    w("   first action when data lands, and it must precede the first real run.")
    w("7. **The MTF bias gate.** `bias.gate_mode = none` throughout (Phases 2-4 unbuilt),")
    w("   so every count here is an upper bound: a real gate can only reduce them.")
    w("8. **Multi-symbol anything.** One symbol, so the correlation cap, the")
